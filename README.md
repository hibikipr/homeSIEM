...
| [`siem-api`](siem-api/) | Go service: rule scheduler (threshold / first-seen / absence rule shapes), alert lifecycle, Loki client, OIDC token verification + RBAC, SQLite state, rich ntfy delivery (severity emoji tags, Markdown body, click-through link, action button, icon), optional LLM-powered insights via Ollama. |
